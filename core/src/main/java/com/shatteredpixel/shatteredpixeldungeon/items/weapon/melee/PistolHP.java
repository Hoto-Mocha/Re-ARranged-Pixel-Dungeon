...
public class PistolHP extends MeleeWeapon {
    ...
    public String info() {
        ...
        if (levelKnown) {
            ...
            info += "\n\n" + Messages.get(PistolHP.class, "stats_known",
                    Bulletmin(PistolHP.this.buffedLvl()),
                    Bulletmax(PistolHP.this.buffedLvl()),
            ...
            info += "\n\n" + Messages.get(PistolHP.class, "stats_unknown",
                    Bulletmin(0),
                    Bulletmax(0),
                    round, max_round, reload_time);
        }
    ...
    public PistolHP.Bullet knockBullet(){
        return new PistolHP.Bullet();
    }
    public class Bullet extends MissileWeapon {
        ...
        public int damageRoll(Char owner) {
            ...
            int bulletdamage = Random.NormalIntRange(Bulletmin(PistolHP.this.buffedLvl()),
                    Bulletmax(PistolHP.this.buffedLvl()));
        ...
        public boolean hasEnchant(Class<? extends Enchantment> type, Char owner) {
            return PistolHP.this.hasEnchant(type, owner);
        }

        @Override
        public int proc(Char attacker, Char defender, int damage) {
            return PistolHP.this.proc(attacker, defender, damage);
        }
        ...
        public float delayFactor(Char user) {
            if (hero.hasTalent(Talent.RECOIL_CONTROL)) {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return PistolHP.this.delayFactor(user)/(2f + 2f * hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                } else {
                    return PistolHP.this.delayFactor(user)/(1f + hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                }
            } else {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return PistolHP.this.delayFactor(user)/2f;
                } else {
                    if (hero.hasTalent(Talent.HOLSTER) && round == max_round) {
                        return PistolHP.this.delayFactor(user)/(1f+hero.pointsInTalent(Talent.HOLSTER));
                    } else
                        return PistolHP.this.delayFactor(user);
                }
            }
        }
        ...
        @Override
        public int STRReq(int lvl) {
            if (PistolHP.this.masteryPotionBonus) {
                return STRReq(tier, PistolHP.this.buffedLvl()) - 2;
            }
            return STRReq(tier, PistolHP.this.buffedLvl());
        }
    ...
    public static class Recipe extends com.shatteredpixel.shatteredpixeldungeon.items.Recipe.SimpleRecipe {
        ...
        {
            inputs =  new Class[]{Pistol.class, HPBullet.class, ArcaneResin.class};
            inQuantity = new int[]{1, 1, 1};

            cost = 0;

            output = PistolHP.class;
            outQuantity = 1;
        }
