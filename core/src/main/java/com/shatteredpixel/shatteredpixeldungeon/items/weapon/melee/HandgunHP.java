...
public class HandgunHP extends MeleeWeapon {
    ...
    public String info() {
        ...
        if (levelKnown) {
            ...
            info += "\n\n" + Messages.get(HandgunHP.class, "stats_known",
                    Bulletmin(HandgunHP.this.buffedLvl()),
                    Bulletmax(HandgunHP.this.buffedLvl()),
            ...
            info += "\n\n" + Messages.get(HandgunHP.class, "stats_unknown",
                    Bulletmin(0),
                    Bulletmax(0),
                    round, max_round, reload_time);
        }
    ...
    public HandgunHP.Bullet knockBullet(){
        return new HandgunHP.Bullet();
    }
    public class Bullet extends MissileWeapon {
        ...
        public int damageRoll(Char owner) {
            ...
            int bulletdamage = Random.NormalIntRange(Bulletmin(HandgunHP.this.buffedLvl()),
                    Bulletmax(HandgunHP.this.buffedLvl()));
        ...
        public boolean hasEnchant(Class<? extends Enchantment> type, Char owner) {
            return HandgunHP.this.hasEnchant(type, owner);
        }

        @Override
        public int proc(Char attacker, Char defender, int damage) {
            return HandgunHP.this.proc(attacker, defender, damage);
        }
        ...
        public float delayFactor(Char user) {
            if (hero.hasTalent(Talent.RECOIL_CONTROL)) {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return HandgunHP.this.delayFactor(user)/(2f + 2f * hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                } else {
                    return HandgunHP.this.delayFactor(user)/(1f + hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                }
            } else {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return HandgunHP.this.delayFactor(user)/2f;
                } else {
                    if (hero.hasTalent(Talent.HOLSTER) && round == max_round) {
                        return HandgunHP.this.delayFactor(user)/(1f+hero.pointsInTalent(Talent.HOLSTER));
                    } else
                        return HandgunHP.this.delayFactor(user);
                }
            }
        }
        ...
        @Override
        public int STRReq(int lvl) {
            if (HandgunHP.this.masteryPotionBonus) {
                return STRReq(tier, HandgunHP.this.buffedLvl()) - 2;
            }
            return STRReq(tier, HandgunHP.this.buffedLvl());
        }
    ...
    public static class Recipe extends com.shatteredpixel.shatteredpixeldungeon.items.Recipe.SimpleRecipe {
        ...
        {
            inputs =  new Class[]{Handgun.class, HPBullet.class, ArcaneResin.class};
            inQuantity = new int[]{1, 1, 1};

            cost = 0;

            output = HandgunHP.class;
            outQuantity = 1;
        }
