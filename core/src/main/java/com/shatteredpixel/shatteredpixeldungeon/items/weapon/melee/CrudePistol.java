...
public class CrudePistol extends MeleeWeapon {
    ...
    public class Bullet extends MissileWeapon {
        ...
        public float delayFactor(Char user) {
            if (hero.hasTalent(Talent.RECOIL_CONTROL)) {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return CrudePistol.this.delayFactor(user)/(2f + 2f * hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                } else {
                    return CrudePistol.this.delayFactor(user)/(1f + hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                }
            } else {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return CrudePistol.this.delayFactor(user)/2f;
                } else {
                    if (hero.hasTalent(Talent.HOLSTER) && round == max_round) {
                        return CrudePistol.this.delayFactor(user)/(1f+hero.pointsInTalent(Talent.HOLSTER));
                    } else
        ...
        @Override
        public int STRReq(int lvl) {
            if (CrudePistol.this.masteryPotionBonus) {
                return STRReq(tier, CrudePistol.this.buffedLvl()) - 2;
            }
