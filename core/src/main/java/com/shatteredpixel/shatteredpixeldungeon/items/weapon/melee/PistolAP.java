...
public class PistolAP extends MeleeWeapon {
    ...
    public String info() {
        ...
        if (levelKnown) {
            ...
            info += "\n\n" + Messages.get(PistolAP.class, "stats_known",
                    Bulletmin(PistolAP.this.buffedLvl()),
            ...
            info += "\n\n" + Messages.get(PistolAP.class, "stats_unknown",
                    Bulletmin(0),
                    Bulletmax(0),
                    round, max_round, reload_time);
        }
    ...
    public PistolAP.Bullet knockBullet(){
        return new PistolAP.Bullet();
    }
    public class Bullet extends MissileWeapon {
        ...
        public int damageRoll(Char owner) {
            ...
            int bulletdamage = Random.NormalIntRange(Bulletmin(PistolAP.this.buffedLvl()),
                    Bulletmax(PistolAP.this.buffedLvl()));
        ...
        public boolean hasEnchant(Class<? extends Enchantment> type, Char owner) {
            return PistolAP.this.hasEnchant(type, owner);
        }

        @Override
        public int proc(Char attacker, Char defender, int damage) {
            return PistolAP.this.proc(attacker, defender, damage);
        }
        ...
        public float delayFactor(Char user) {
            if (hero.hasTalent(Talent.RECOIL_CONTROL)) {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return PistolAP.this.delayFactor(user)/(2f + 2f * hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                } else {
                    return PistolAP.this.delayFactor(user)/(1f + hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                }
            } else {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return PistolAP.this.delayFactor(user)/2f;
                } else {
                    if (hero.hasTalent(Talent.HOLSTER) && round == max_round) {
                        return PistolAP.this.delayFactor(user)/(1f+hero.pointsInTalent(Talent.HOLSTER));
                    } else
                        return PistolAP.this.delayFactor(user);
                }
            }
        }
        ...
        @Override
        public int STRReq(int lvl) {
            if (PistolAP.this.masteryPotionBonus) {
                return STRReq(tier, PistolAP.this.buffedLvl()) - 2;
            }
            return STRReq(tier, PistolAP.this.buffedLvl());
        }
    ...
    public static class Recipe extends com.shatteredpixel.shatteredpixeldungeon.items.Recipe.SimpleRecipe {
        ...
        {
            inputs =  new Class[]{Pistol.class, APBullet.class, ArcaneResin.class};
            inQuantity = new int[]{1, 1, 1};

            cost = 0;

            output = PistolAP.class;
            outQuantity = 1;
        }
