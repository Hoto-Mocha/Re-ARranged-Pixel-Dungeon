...
public class LargeHandgunHP extends MeleeWeapon {
        ...
        public float delayFactor(Char user) {
            if (hero.hasTalent(Talent.RECOIL_CONTROL)) {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return LargeHandgunHP.this.delayFactor(user)/2f*(1f + hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                } else {
    ...
    public static class Recipe extends com.shatteredpixel.shatteredpixeldungeon.items.Recipe.SimpleRecipe {
        ...
        {
            inputs =  new Class[]{LargeHandgun.class, HPBullet.class, ArcaneResin.class};
            inQuantity = new int[]{1, 1, 1};

            cost = 0;

            output = LargeHandgunHP.class;
            outQuantity = 1;
        }
