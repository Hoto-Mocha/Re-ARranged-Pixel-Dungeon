...
public class GoldenPistolAP extends MeleeWeapon {
    ...
    public String info() {
        ...
        if (levelKnown) {
            ...
            info += "\n\n" + Messages.get(GoldenPistolAP.class, "stats_known",
                    Bulletmin(GoldenPistolAP.this.buffedLvl()),
                    Bulletmax(GoldenPistolAP.this.buffedLvl()),
            ...
            info += "\n\n" + Messages.get(GoldenPistolAP.class, "stats_unknown",
                    Bulletmin(0),
                    Bulletmax(0),
                    round, max_round, reload_time);
        }
    ...
    public GoldenPistolAP.Bullet knockBullet(){
        return new GoldenPistolAP.Bullet();
    }
    public class Bullet extends MissileWeapon {
        ...
        public int damageRoll(Char owner) {
            ...
            int bulletdamage = Random.NormalIntRange(Bulletmin(GoldenPistolAP.this.buffedLvl()),
                    Bulletmax(GoldenPistolAP.this.buffedLvl()));
        ...
        public boolean hasEnchant(Class<? extends Enchantment> type, Char owner) {
            return GoldenPistolAP.this.hasEnchant(type, owner);
        }

        @Override
        public int proc(Char attacker, Char defender, int damage) {
            return GoldenPistolAP.this.proc(attacker, defender, damage);
        }
        ...
        public float delayFactor(Char user) {
            if (hero.hasTalent(Talent.RECOIL_CONTROL)) {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return GoldenPistolAP.this.delayFactor(user)/(2f + 2f * hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                } else {
                    return GoldenPistolAP.this.delayFactor(user)/(1f + hero.pointsInTalent(Talent.RECOIL_CONTROL)/3f);
                }
            } else {
                if (hero.buff(Riot.riotTracker.class) != null) {
                    return GoldenPistolAP.this.delayFactor(user)/2f;
                } else {
                    if (hero.hasTalent(Talent.HOLSTER) && round == max_round) {
                        return GoldenPistolAP.this.delayFactor(user)/(1f+hero.pointsInTalent(Talent.HOLSTER));
                    } else
                        return GoldenPistolAP.this.delayFactor(user);
                }
            }
        }
    ...
    public static class Recipe extends com.shatteredpixel.shatteredpixeldungeon.items.Recipe.SimpleRecipe {
        ...
        {
            inputs =  new Class[]{GoldenPistol.class, APBullet.class, ArcaneResin.class};
            inQuantity = new int[]{1, 1, 1};

            cost = 0;

            output = GoldenPistolAP.class;
            outQuantity = 1;
        }
