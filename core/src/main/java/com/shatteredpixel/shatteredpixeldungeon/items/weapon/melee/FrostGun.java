...
import com.shatteredpixel.shatteredpixeldungeon.actors.buffs.FrostDartCoolDown;
...
public class FrostGun extends MeleeWeapon {
...

    {
        ...
        image = ItemSpriteSheet.FROST_GUN;
    ...
    public void execute(Hero hero, String action) {
        ...
        if (action.equals(AC_SHOOT)) {
            if (round <= 0) {
                reload_time = 1f;
                if (hero.buff(FrostDartCoolDown.class) != null) {
                    usesTargeting = false;
                    GLog.w(Messages.get(FrostGun.class, "cannot_reload"));
                } else {
                    reload();
                }
            } else {
    ...
    public String info() {
        ...
        if (levelKnown) {
            info += "\n\n" + Messages.get(FrostGun.class, "stats_known",
                    Bulletmin(FrostGun.this.buffedLvl()),
                    Bulletmax(FrostGun.this.buffedLvl()),
    ...
    public FrostGun.Dart knockBullet(){
        return new FrostGun.Dart();
    }
    public class Dart extends MissileWeapon {
        ...
        @Override
        public int buffedLvl(){
            return FrostGun.this.buffedLvl();
        }
        ...
        public int damageRoll(Char owner) {
            ...
            int bulletdamage = Random.NormalIntRange(Bulletmin(FrostGun.this.buffedLvl()),
                    Bulletmax(FrostGun.this.buffedLvl()));
        ...
        public boolean hasEnchant(Class<? extends Enchantment> type, Char owner) {
            return FrostGun.this.hasEnchant(type, owner);
        }
        ...
        public int proc(Char attacker, Char defender, int damage) {
            ...
            if (FrostGun.this.enchantment == null
            ...
            } else if (FrostGun.this.enchantment == null
                    && Random.Int(3) < hero.pointsInTalent(Talent.SHARED_ENCHANTMENT)
                    && hero.buff(MagicImmune.class) == null
                    && bow2 != null
                ...
                return FrostGun.this.proc(attacker, defender, damage);
            }
        }
        ...
        public float delayFactor(Char user) {
            if (hero.buff(Riot.riotTracker.class) != null) {
                return FrostGun.this.delayFactor(user)/2f;
            } else {
                return FrostGun.this.delayFactor(user);
            }
        }

        @Override
        public int STRReq(int lvl) {
            if (FrostGun.this.masteryPotionBonus) {
                return STRReq(tier, FrostGun.this.buffedLvl()) - 2;
            }
            return STRReq(tier, FrostGun.this.buffedLvl());
        }

        @Override
        protected void onThrow( int cell ) {
            float duration = FrostDartCoolDown.DURATION - 4*FrostGun.this.buffedLvl();
            if (FrostGun.this.buffedLvl() >= 6) {
                duration -= 16;
            }
            Buff.prolong(hero, FrostDartCoolDown.class, duration);
